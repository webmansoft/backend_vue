...
const odata = isDevelop ? {username: 'admin', password: '123456', captcha: ''} : {username: '', password: '', captcha: ''};
  ...
  <div class="login-container">
    <div class="login-width md:w-10/12 w-11/12 mx-auto flex justify-between h-full items-center">
      <div class="w-6/12 mx-auto left-panel rounded-l pl-5 pr-5 hidden md:block">
        <div class="logo">
          <img :src="`${$url}logo.png`" width="45" alt="logo"/><span>{{ $title }}</span>
        </div>
      ...
      <div class="md:w-6/12 w-11/12 md:rounded-r mx-auto pl-5 pr-5 pb-10">
        ...
        <a-form :model="form" @submit="handleSubmit">
          ...
          >
            <a-input v-model="form.captcha" :placeholder="$t('sys.login.verifyCode')" size="large" allow-clear>
              <template #prefix>
                <icon-safe/>
              </template>
              <template #append>
                <img :src="captcha" style="width: 120px; height: 36px; cursor: pointer" @click="refreshCaptcha"
                     alt="验证码"/>
              </template>
            </a-input>
          </a-form-item>
