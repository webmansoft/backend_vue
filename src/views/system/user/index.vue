...
const dictionarys = ref([{label: '所有部门', value: 0}])
...
onMounted(() => {
  dictionary.tree().then(res => {
    res.data.map(item => {
      dictionarys.value.push(item)
    })
  })
...
const switchDept = (id) => {
  crudRef.value.requestParams = id[0] === 0 ? {dictionary_id: undefined} : {dictionary_id: id[0]}
...
const crud = reactive({
  ...
  recovery: {show: true, api: user.recovery, auth: ['/core/user/recovery']},
  formOption: {
    width: 800,
    layout: [
      ...
      {
        formType: 'grid',
        cols: [{span: 12, formList: [{dataIndex: 'username'}]}, {span: 12, formList: [{dataIndex: 'dept_id'}]}]
      },
      {
        formType: 'grid',
